...
function checkForUserApiKey(){
    ...
    if(!apiKey) {
        ...
        let input = document.getElementById("api-key-input") as HTMLInputElement;
        input.value = apiKey;
...
function setupEvents(){
    searchForm?.addEventListener("submit", (e) => {
        e.preventDefault();
        let searchInput = searchForm.querySelector<HTMLInputElement>("#search-input")?.value.trim();
        if(searchInput)
...
function saveApiKey(){
    apiKey = apiKeyForm.querySelector<HTMLInputElement>("#api-key-input")?.value.trim() ?? "";
...
async function renderLocationOnPage(searchInput:string){
    let locationData:IpLocation|null = await searchLocation(searchInput);
...
async function searchLocation(input:string){
    ...
    let locationData:IpLocation|null = await fetchData<IpLocation>(url);
